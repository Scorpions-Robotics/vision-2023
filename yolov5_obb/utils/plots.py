...
class Annotator:
    ...
    def poly_label(
        ...
        self.im = (
            self.im_cv2
            if isinstance(self.im_cv2, Image.Image)
            else Image.fromarray(self.im_cv2)
        )
...
def plot_images(
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            ti = targets[
                targets[:, 0] == i
            ]  # image targets, (n, [img_index clsid cx cy l s theta gaussian_θ_labels])
