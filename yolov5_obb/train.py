...
def train(hyp, opt, device, callbacks):  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            labels = np.concatenate(
                dataset.labels, 0
            )  # labels(array): (all_images_gt_num, [cls_id, poly])
    ...
    results = (
        ...
        0,
        0,
    )  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls, theta)
    ...
    for epoch in range(
        ...
        for i, (
            ...
            if opt.multi_scale and not opt.rect:
                ...
                sf = sz / max(
                    imgs.shape[2:]
                )  # scale factor , img (tensor): (b, 3, height, width)
                if sf != 1:
                    ns = [
                        math.ceil(x * sf / gs) * gs for x in imgs.shape[2:]
                    ]  # new shape (stretched to gs-multiple) [h_new, w_new]
                    ...
                    targets[
                        :, 2:6
                    ] *= label_ratio  # targets (tensor): (n_targets, [img_index clsid cx cy l s theta gaussian_θ_labels])
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default=ROOT / "weights/yolov5n.pt",
    ...
    parser.add_argument(
        "--hyp",
        type=str,
        default=ROOT / "data/hyps/obb/hyp.finetune_dota.yaml",
