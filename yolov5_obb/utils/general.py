...
def clip_polys(polys, shape):
    ...
    if isinstance(polys, torch.Tensor):  # faster individually
        ...
        polys[:, [0, 2, 4, 6]] = polys[:, [0, 2, 4, 6]].clip(
            0, shape[1]
        )  # x1, x2, x3, x4
        polys[:, [1, 3, 5, 7]] = polys[:, [1, 3, 5, 7]].clip(
            0, shape[0]
        )  # y1, y2, y3, y4
...
def non_max_suppression_obb(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = x[
            xc[xi]
        ]  # confidence, (tensor): (n_conf_thres, [cx cy l s obj num_cls theta_cls])
        ...
        _, theta_pred = torch.max(
            x[:, class_index:], 1, keepdim=True
        )  # [n_conf_thres, 1] θ ∈ int[0, 179]
        ...
        if multi_label:
            ...
            x = torch.cat(
                (x[i, :4], theta_pred[i], x[i, j + 5, None], j[:, None].float()), 1
            )
        else:  # best class only
            conf, j = x[:, 5:class_index].max(1, keepdim=True)
            x = torch.cat((x[:, :4], theta_pred, conf, j.float()), 1)[
