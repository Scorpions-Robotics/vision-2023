...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    ls_edges = []
    for ratio, labels in zip(
        min_ratios * scales, dataset.labels
...
def kmean_anchors(
    ...
    if i:
        LOGGER.info(
            f"{PREFIX}WARNING: Extremely small objects found. {i} of {len(ls_edges0)} poly labels are < 5 pixels in size."
        )
