...
def save_one_json(pred_hbbn, pred_polyn, jdict, path, class_map):
    ...
    for p, b in zip(pred_polyn.tolist(), box.tolist()):
        jdict.append(
            {
                "image_id": image_id,
                "category_id": class_map[
                    int(p[-1]) + 1
                ],  # COCO's category_id start from 1, not 0
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        )  # list*(n, [xylsθ, conf, cls]) θ ∈ [-pi/2, pi/2)
        dt[2] += time_sync() - t3
        ...
        for si, pred in enumerate(out):  # pred (tensor): (n, [xylsθ, conf, cls])
            labels = targets[
                targets[:, 0] == si, 1:7
            ]  # labels (tensor):(n_gt, [clsid cx cy l s theta]) θ[-pi/2, pi/2)
            nl = len(labels)
            tcls = labels[:, 0].tolist() if nl else []  # target class
            path, shape = (
                Path(paths[si]),
                shapes[si][0],
            )  # shape (tensor): (h_raw, w_raw)
            seen += 1
            ...
            pred_hbb = torch.cat(
                (hbbox, pred_poly[:, -2:]), dim=1
            )  # (n, [xyxy, conf, cls])

            pred_polyn = pred_poly.clone()  # predn (tensor): (n, [poly, conf, cls])
            scale_polys(
                im[si].shape[1:], pred_polyn[:, :8], shape, shapes[si][1]
            )  # native-space pred
            hbboxn = xywh2xyxy(poly2hbb(pred_polyn[:, :8]))  # (n, [x1 y1 x2 y2])
            pred_hbbn = torch.cat(
                (hbboxn, pred_polyn[:, -2:]), dim=1
            )  # (n, [xyxy, conf, cls]) native-space pred
            ...
            if nl:
                ...
                labels_hbbn = torch.cat(
                    (labels[:, 0:1], tbox), 1
                )  # native-space labels (n, [cls xyxy])
            ...
            stats.append(
                (correct.cpu(), pred_poly[:, 8].cpu(), pred_poly[:, 9].cpu(), tcls)
            ...
            if save_json:  # save hbb pred results and poly pred results.
                save_one_json(
                    pred_hbbn, pred_polyn, jdict, path, class_map
...
def parse_opt():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default=ROOT / "data/DroneVehicle_poly.yaml",
    ...
    parser.add_argument(
        "--conf-thres", type=float, default=0.01, help="confidence threshold"
    )
    parser.add_argument(
        "--iou-thres", type=float, default=0.4, help="NMS IoU threshold"
    )
    parser.add_argument(
