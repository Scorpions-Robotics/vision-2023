...
def results_ensemble(srcpath_1, srcpath_2, dstpath):
    ...
    filelist_1 = GetFileFromThisRootDir(
        srcpath_1
    )  # srcpath文件夹下的所有文件相对路径 eg:['Task1_??.txt', ..., '?.txt']
...
def parse_args():
    ...
    parser.add_argument(
        "--srcpath_1",
        default="/OrientedRepPoints/tools/parse_pkl/evaluation_results/ORR_results/",
        help="srcpath_1",
    )
    parser.add_argument(
        "--srcpath_2",
        default="/OrientedRepPoints/tools/parse_pkl/evaluation_results/ROI_results/",
        help="srcpath_2",
    )
    parser.add_argument(
        "--dstpath",
