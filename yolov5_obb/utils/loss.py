...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, indices, anchors, tgaussian_theta = self.build_targets(
            p, targets
        )  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ps = pi[
                    b, a, gj, gi
                ]  # prediction subset corresponding to targets, (n_targets, self.no)
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    t = torch.full_like(
                        ps[:, 5:class_index], self.cn, device=device
                    )  # targets
                ...
                t_theta = tgaussian_theta[i].type(
                    ps.dtype
                )  # target theta_gaussian_labels
        ...
        return (lbox + lobj + lcls + ltheta) * bs, torch.cat(
            (lbox, lobj, lcls, ltheta)
        ).detach()
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                r = (
                    t[:, :, 4:6] / anchors[:, None]
                )  # edge_ls ratio, torch.size(na, n_gt_all_batch, 2)
                j = (
                    torch.max(r, 1 / r).max(2)[0] < self.hyp["anchor_t"]
                )  # compare, torch.size(na, n_gt_all_batch)
                ...
                t = t.repeat((5, 1, 1))[
                    j
                ]  # (n_filter1, c+1) -> (5, n_filter1, c+1) -> (n_filter2, c+1)
                offsets = (torch.zeros_like(gxy)[None] + off[:, None])[
                    j
                ]  # (5, n_filter1, 2) -> (n_filter2, 2)
            else:
